...
export default function Projects() {
  ...
  if (loading) {
    return (
      <section id="projects" className="py-12 md:py-20 relative overflow-hidden bg-white">
    ...
    return (
      <section id="projects" className="py-12 md:py-20 relative overflow-hidden bg-white">
        <div className="container mx-auto px-6 relative z-10 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Featured Projects</h2>
          <p className="text-lg md:text-xl text-red-600 mb-8">{error}</p>
          <button 
  ...
  if (projects.length === 0) {
    return (
      <section id="projects" className="py-12 md:py-20 relative overflow-hidden bg-white">
        <div className="container mx-auto px-6 relative z-10 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">Featured Projects</h2>
          <p className="text-lg md:text-xl text-gray-600">No projects available yet.</p>
        </div>
      </section>
    )
  }
          ...
          {projects.map((project, index) => (
            ...
            >
              ...
              <div className="relative overflow-hidden">
                <div className="w-full h-40 md:h-48 bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center">
                  {/* Icon size responsive using Tailwind classes */}
              ...
              <div className="p-4 md:p-6">
                <div className="flex items-start justify-between mb-2">
                  ...
                  {project.featured && (
                    <span className="flex-shrink-0 text-[10px] md:text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">
                      Featured
                    </span>
                  )}
                </div>
                ...
                <p className="text-sm md:text-base text-gray-600 mb-4 leading-relaxed line-clamp-3 h-[4.5em]">
                  {project.description}
                </p>

                {/* Tech Tags - Compact for mobile */}
                <div className="flex flex-wrap gap-1.5 md:gap-2 mb-4 h-[28px] md:h-[32px] overflow-hidden">
                  {project.technologies.slice(0, 3).map((tech) => (
                    <span
                      key={tech}
                      className="bg-blue-100 text-blue-700 px-2.5 py-0.5 md:px-3 md:py-1 rounded-full text-xs md:text-sm font-medium whitespace-nowrap"
                    >
                      {tech}
                    </span>
                  ))}
                  {project.technologies.length > 3 && (
                    <span className="bg-gray-100 text-gray-600 px-2 py-0.5 md:py-1 rounded-full text-[10px] md:text-xs flex items-center">
                      +{project.technologies.length - 3}
                    </span>
                  )}
                </div>

                {/* Results Section - Tighter spacing for mobile */}
                <div className="space-y-1.5 md:space-y-2 border-t border-gray-100 pt-3 md:pt-4">
